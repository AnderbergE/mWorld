...
BirdheroGame.prototype.create = function () {
	...
	this.speech = util.createAudioSheet('birdheroSpeech', LANG.SPEECH.birdhero.markers);
	this.sfx = util.createAudioSheet('birdheroSfx', {
		intro:          [ 0.0, 12.7],
		happy:          [13.4,  3.8],
		outro:          [18.0,  3.7],
	...
	for (var i = 0; i < this.amount; i++) {
		this.tree.branch[i] = new BirdheroBranch(this.game, treeCenter, treeBottom - heightPerBranch*i, this.tint[i]);
...
BirdheroGame.prototype.newRound = function (silent) {
	...
	t.addCallback(this.agent.eyesFollowObject, 0, [this.bird.beak], this.agent);
...
BirdheroGame.prototype.runNumber = function (number, simulate) {
	...
	t.addCallback(this.agent.eyesFollowObject, null, [this.bird.beak], this.agent);
	...
	/* Incorrect :( */
		...
		t.addSound(this.speech, this.bird, this.rnd.integerInRange(0, 1) ? 'dontLiveHere' : 'notMyParent');
...
BirdheroGame.prototype.modeIntro = function () {
	...
	// Make it dark!
	var darkness = new Cover(this.game, '#000077', 0);
	group.add(darkness);
...
BirdheroGame.prototype.modePlayerShow = function (intro, tries) {
	if (tries > 0) {
		...
		if (intro) {
			...
			t.addSound(this.agent.speech, this.agent, 'birdheroIntro', 'agentIntro');
 ...
 */
function BirdheroBranch (game, x, y, tint) {
...
BirdheroBranch.prototype.confused = function (duration) {
	if (!this.confusing) {
		this.confusing = this.game.add.text(this.mother.x+10, this.mother.y-40, '?!?', {
